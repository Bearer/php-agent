...
class HookProcessor
{
	...
	/**
	 * HookProcessor constructor.
	 */
	public function __construct($handlers = null)
	{
		if (!isset(static::$transformers[Transformer::NAME])) {
			static::$transformers[Transformer::NAME] = (new Transformer($handlers))->register();
		}
	}
	...
	protected function appendFiltersToStream($stream)
	{
		foreach (static::$transformers as $transformer) {
			stream_filter_append($stream, $transformer::NAME, STREAM_FILTER_READ);
		}
	}
